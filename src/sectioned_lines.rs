    ...
    F: Fn(&str) -> Option<Section>,
{
    ...
    pub fn next_section(&mut self) -> Option<Section> {
        self.found.take().or_else(|| find_section(&mut self.lines, &self.section_finder))
    }
}
...
where
    ...
    F: Fn(&str) -> Option<Section>,
{
    Iter {found: None, lines, section_finder}
}


/// Auxiliary function that finds the next section delimiter if any.
fn find_section<'a, Section, Lines, F>(
    lines: &mut Lines,
    section_finder: &'a F,
) -> Option<Section>
where
    ...
    <Lines as Iterator>::Item: 'a,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.found.is_some() {return None}
